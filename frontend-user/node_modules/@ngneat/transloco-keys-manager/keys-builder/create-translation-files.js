...
function createTranslationFiles({ scopeToKeys, langs, output, replace, scopes, fileFormat, }) {
    ...
    if (newFiles.length) {
        logger.success(`${messages_1.messages.creatingFiles} 🗂`);
        logger.log(newFiles.map((action) => action.path).join('\n'));
    }
    logger.log(`\n              🌵 ${messages_1.messages.done} 🌵`);
}
